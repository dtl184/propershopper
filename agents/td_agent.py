...
class TDAgent:
    ...
    def __init__(self, goal=None, alpha=0.1, gamma=0.9, epsilon=0.3, mini_epsilon=0.05, decay=0.9999, type=None, n=1):
        """
    ...
    def learning(self, action, state, next_state, reward):
        ...
        if self.type == 'Q':
            ...
            act = self.choose_action(next_state)
            next_q_sa = self.qtable[next_state_idx][act]
            self.qtable[state_idx][action] += self.alpha * (reward + self.gamma * next_q_sa - q_sa)
        
        return act
    
    def nstep_learning(self, action, state, next_state, t, T):
        """
        n step sarsa
        """
        state_idx = self.state_index(state)
        ...
        tau = t - self.n + 1
        if tau >= 0:
            G = self.nstep_return(tau, T)
            if tau + self.n < T:
                G += (self.gamma ** self.n) * self.qtable[self.trajectory[t][0]][self.trajectory[t][1]] #discounted Q(s_t, a_t)
            old_q = self.qtable[self.trajectory[tau][0]][self.trajectory[tau][1]]
            self.qtable[self.trajectory[tau][0]][self.trajectory[tau][1]] += self.alpha * (G - old_q)

    
    def nstep_return(self, tau, T):
        G = 0
        for i in range(0, self.n):
            G += (self.gamma ** i) * self.trajectory[i][2]
            if self.n + tau == T:
                break
        return G
